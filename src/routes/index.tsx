...
function Login() {
  ...
  function onSubmit(e: FormEvent) {
    e.preventDefault();
    setLoading(true);
    // Milestone 1: no auth logic yet — cinematic transition into the OS.
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-black text-white">
      <img
        ...
        width={1920}
        height={1200}
        className={`absolute inset-0 h-full w-full object-cover transition-all duration-[1200ms] ${loading ? "scale-105 blur-sm" : "scale-100"}`}
      />
      <div className="absolute inset-0 bg-gradient-to-b from-black/50 via-black/25 to-black/70" />

      <div className={`relative z-10 min-h-screen flex flex-col items-center justify-center px-6 transition-opacity duration-500 ${loading ? "opacity-0" : "opacity-100"}`}>
        ...
        <form onSubmit={onSubmit} className="w-full max-w-sm space-y-5">
          ...
          <button
            type="submit"
            className="w-full rounded-full bg-white text-neutral-900 py-3.5 text-sm font-medium tracking-wide hover:bg-white/90 transition"
          >
            Sign In
          </button>
        </form>
